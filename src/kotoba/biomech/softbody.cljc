(ns kotoba.biomech.softbody
  "Soft biological tissue (muscle, skin, organ wall) as a 3-D mass-spring-damper
  grid, built on kotoba-lang/kami-vehicle's vec3 / node / beam primitives.

  kami-vehicle's `vehicle.vehicle/step` is vehicle-specific (tire / powertrain /
  Pacejka magic formula), so this namespace consumes only the data primitives
  (vehicle.vec3 / vehicle.node / vehicle.beam) and runs its own beam-only
  integrator — kotoba.biomech.muscle's semi-implicit Euler lifted to 3-D, with
  per-beam spring stiffness k [N/m] and viscous damping c [N·s/m]. Gravity is
  configurable; anchor nodes (mass 0) are fixed in place."
